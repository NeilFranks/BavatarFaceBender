...
DC = 3  # direct current, will keep rolling average
DCRollLength = 200
    ...
    if peak >= 12:  # start getting mostly unnique notes at 12
        for i in range(len(noteArr)):
            for note in noteArr[i]:
                # print "note:" + str(note)
                if peak > .9727 * note and peak < 1.029 * note:
                    print(noteNames[i])
...
def findNotes(peakInds, peakVals):
    ...
    lowValWeight = .15  # for weighting lower freqs more than higher freqs
    ...
    for i in range(N):
        for j in range(numPeaks):
            for note in noteArr[i]:
                ...
                if peakInds[j] > .9727 * note and peakInds[j] < 1.029 * note:
                    # add the strength of the freq to the note in noteCount
                    noteCount[i] += peakVals[j] * \
                        ((dfftLengthLog2 -
                          np.log2(peakInds[j])) / dfftLengthLog2)**2
                    #noteCount[i] += 1
...
#     print noteCount
    ...
    unique = False  # unique indices are 15 and above. everything below that is triggered by 2+ noteArr
    for ind in peakInds:
        if ind >= 12:
            unique = True
...
def countCrosses(freqs, limit):
    ...
    for idx in range(N - 1):
        if (freqs[idx] > limit and freqs[idx + 1] <= limit) or (freqs[idx] <= limit and freqs[idx + 1] > limit):
            crosses += 1
...
class filter:  # filter is used to only respond to certain frequencies at certain strengths
    ...
    def draw(self):
        ...
        pygame.draw.rect(display, (0, 255, 255), Rect(
            self.start[0] - self.handleSize / 2, self.start[1] - self.handleSize / 2, self.handleSize, self.handleSize))

        # draw end point handle
        pygame.draw.rect(display, (0, 255, 255), Rect(
            self.end[0] - self.handleSize / 2, self.end[1] - self.handleSize / 2, self.handleSize, self.handleSize))
    ...
    def task(self, subDfft, filter, rangeStart, totalPeaks):
        ...
        for peakIdx in totalPeaks[0]:
            if rangeStart < peakIdx < rangeStart + len(subDfft) and subDfft[peakIdx - rangeStart] > filter:
        ...
        if len(peaksInRange) > 0:
            ...
            pygame.draw.rect(display, (100, 100, 255), Rect(
                self.start[0] + LRPercent * (self.end[0] - self.start[0]), 50, 30, area))
            pygame.display.flip()
    ...
    def inStart(self, coordinates):
        if self.start[0] - self.handleSize < coordinates[0] < self.start[0] + self.handleSize and self.start[1] - self.handleSize < coordinates[1] < self.start[1] + self.handleSize:
            return True
        else:
            return False

    def inEnd(self, coordinates):
        if self.end[0] - self.handleSize < coordinates[0] < self.end[0] + self.handleSize and self.end[1] - self.handleSize < coordinates[1] < self.end[1] + self.handleSize:
            return True
        else:
            return False
...
#                      print q
        ...
        for f in filters:
            ...
            if f.end[0] * dfftLength / width - f.start[0] * dfftLength / width > 0:
                maxInRange = max(
                    dfft[round(f.start[0] * dfftLength / width):round(f.end[0] * dfftLength / width)])  # compensate for how dfft was displayed

                maxIndex = list(dfft).index(maxInRange)

                diff = f.start[1] - (height - (dfft[maxIndex] - 50)**2)
                if diff > 0:
                    # send (dfft values in range, equivalent cutoff due to
                    # filter) to a task
                    rangeStart = round(f.start[0] * dfftLength / width)
                    rangeEnd = round(f.end[0] * dfftLength / width)

                    relativeFilter = float(
                        height - f.start[1])**(0.6667) + 50

                    totalPeaks = findPeaks(dfft, 0, 0)

                    f.task(dfft[rangeStart:rangeEnd],
                           relativeFilter, rangeStart, totalPeaks)
...
#     device = p.get_device_info_by_index(i)
#     if device['maxInputChannels'] > 0:
#         audio_devices.append((device['name'], i))
#     i += 1
